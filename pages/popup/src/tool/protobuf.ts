...
export const base32Encode = (bytes: Uint8Array) => {
  ...
  // Tùy chọn: padding "=", Google Authenticator không cần
  return output;
};
...
export const parseMigrationQRCode = (qrURL: string) => {
  ...
  if (!base64Data) throw new Error('Nội dung QR không hợp lệ, vui lòng quét lại');
    ...
    if (field === 1 && wireType === 2) {
      ...
      while (pPos < paramBytes.length) {
        ...
        if (w === 2) {
          ...
          if (f === 1) account.secretBytes = raw; // giữ nguyên bytes gốc
